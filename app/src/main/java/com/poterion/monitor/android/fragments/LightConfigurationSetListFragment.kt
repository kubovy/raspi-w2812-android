...
import com.poterion.monitor.android.adapters.LightConfigurationSetListAdapter
...
class LightConfigurationSetListFragment : Fragment(), Page {
	...
	override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
		...
		realm?.where(RealmLightConfiguration::class.java)
				...
				?.findAllAsync()
				?.also { sets ->
					root.listView.adapter = LightConfigurationSetListAdapter(sets, {
						interaction?.set ?: ""
					}) { set ->
						realm?.where(RealmLightConfiguration::class.java)
								?.equalTo("set", set)
	...
	override fun onCreateOptionsMenu(menu: Menu?, inflater: MenuInflater?) {
		...
		menu?.findItem(R.id.action_clone)?.isVisible = false

...
import com.poterion.raspi.w2812.android.adapters.SectionsPagerAdapter
...
class MainActivity : AppCompatActivity(), ActivityCommunicationInterface {
	...
	override fun sendLightConfigurations(lightConfigurations: List<RealmLightConfiguration>) {
		bluetoothSerial?.also {
			LightUpdateTask(it) {
				if (!it) Toast.makeText(this, getString(R.string.error_changing_light_failed), Toast.LENGTH_LONG).show()
			}.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR,
					*lightConfigurations.map { it.toLightConfiguration() }.toTypedArray())
		}
	}

	private fun goto(position: Int) = if (container.currentItem != position) {
